...
const Service = () => {
  ...
  return (
    <div className='myContainer'>
        <h2 className='max-w-[650px]'>Everything you need to build high-quality generative AI at scale</h2>
        <div className='myFlex gap-3 mt-6 flex-wrap'>
            {serviceFilter.map((item, index) => (
                <Button cta={item} key={index} className={`hover:bg-cerise hover:text-white ${active === item ? "bg-cerise text-white" : "bg-dimGray"}`} polycolor='bg-white' action={() => setActive(item)}/>
            ))}
        </div>
